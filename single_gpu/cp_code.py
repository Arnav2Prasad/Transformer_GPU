...
'''
The Problem Context Parallelism Solves:
    Without Context Parallelism:
    ┌─────────────────────────────────────────────────┐
    │ GPU Memory Limits: 80GB                          │
    │ Max Sequence Length: 8K tokens                   │
    │ Why? Attention matrix: 8K × 8K = 64M elements   │
    │      KV Cache: 8K × ModelDim                     │
    └─────────────────────────────────────────────────┘

    With Context Parallelism (8 GPUs):
    ┌─────────────────────────────────────────────────┐
    │ Each GPU: 80GB / 8 = "Effective" 640GB memory   │
    │ Max Sequence Length: 64K tokens! (8× increase)  │
    │ Each GPU handles: 8K tokens locally             │
    │ Attention: 8K × 64K (but distributed)           │
    └─────────────────────────────────────────────────┘
...
Concrete Benefits
    1. Memory Scaling
        ...
        Memory per GPU WITHOUT context parallel:
            - Attention matrices: 32K × 32K × 32 heads × 4 bytes ≈ 128GB ❌ (Fits? NO)

        Memory per GPU WITH context parallel (8 GPUs):
            - Local attention: 4K × 32K × 32 heads × 4 bytes ≈ 16GB ✅ (Fits! YES)


Comparison with Other Parallelism Strategies:
    1. Data Parallelism:
        All GPUs get FULL sequence, different data batches
        Good for: Increasing batch size
        Bad for: Long sequences (memory limit per GPU still applies)

    2. Tensor Parallelism:
            Splits model weights across GPUs  
            Good for: Large models (70B+ parameters)
            Bad for: Long sequences (communication overhead)

    3. Pipeline Parallelism:
            Splits model layers across GPUs
            Good for: Very deep models
            Bad for: Training efficiency (bubble overhead)

    4. Context Parallelism:
            Splits SEQUENCE across GPUs
            Good for: VERY long sequences
            Bad for: Small sequences (communication overhead)
'''


'''
Before Context Parallelism

    Single GPU Processing:
    ┌─────────────────────────────────────────────────┐
    │                FULL SEQUENCE                    │
    │ Token0  Token1  Token2  ...  Token1023         │
    │    ↓       ↓       ↓           ↓               │
    │   Q,K,V   Q,K,V   Q,K,V       Q,K,V           │
    │    │       │       │           │               │
    │    └─── Attention over all 1024 tokens ───┘    │
    └─────────────────────────────────────────────────┘


With Context Parallelism (4 GPUs Example)

    Step 1: Sequence Distribution
        GLOBAL SEQUENCE: [T0 T1 T2 T3 T4 T5 T6 T7]

        GPU0 (rank=0): [T0 T1]  ← Local chunk (2 tokens)
        GPU1 (rank=1): [T2 T3]  ← Local chunk (2 tokens)  
        GPU2 (rank=2): [T4 T5]  ← Local chunk (2 tokens)
        GPU3 (rank=3): [T6 T7]  ← Local chunk (2 tokens)

    Step 2: Local Computation (Each GPU):
            GPU0:                            GPU1:
        ┌───────────────┐                ┌───────────────┐
        │ Input: [T0 T1]│                │ Input: [T2 T3]│
        │               │                │               │
        │ Q0 = W_q(T0)  │                │ Q2 = W_q(T2)  │
        │ Q1 = W_q(T1)  │                │ Q3 = W_q(T3)  │
        │               │                │               │
    ...
    Step 3: All-Gather Phase (Key Step!):

        Before All-Gather:
                GPU0 has: K0, K1, V0, V1           (tokens T0-T1)
                GPU1 has: K2, K3, V2, V3           (tokens T2-T3)  
                GPU2 has: K4, K5, V4, V5           (tokens T4-T5)
                GPU3 has: K6, K7, V6, V7           (tokens T6-T7)
            
        All-Gather Operation:
                GPU0 SENDS: K0,K1,V0,V1 → GPU1, GPU2, GPU3
                GPU1 SENDS: K2,K3,V2,V3 → GPU0, GPU2, GPU3  
                GPU2 SENDS: K4,K5,V4,V5 → GPU0, GPU1, GPU3
                GPU3 SENDS: K6,K7,V6,V7 → GPU0, GPU1, GPU2

        After All-Gather:
            EVERY GPU NOW HAS COMPLETE GLOBAL CONTEXT:

                GPU0: [K0 K1 K2 K3 K4 K5 K6 K7], [V0 V1 V2 V3 V4 V5 V6 V7]  ← ALL tokens!
                GPU1: [K0 K1 K2 K3 K4 K5 K6 K7], [V0 V1 V2 V3 V4 V5 V6 V7]  ← ALL tokens!
                GPU2: [K0 K1 K2 K3 K4 K5 K6 K7], [V0 V1 V2 V3 V4 V5 V6 V7]  ← ALL tokens!  
                GPU3: [K0 K1 K2 K3 K4 K5 K6 K7], [V0 V1 V2 V3 V4 V5 V6 V7]  ← ALL tokens!

    Step 4: Attention with Rectangular Causal Masking:
    The Challenge:
        GPU1 handles tokens T2 and T3, but now has ALL keys. We need to ensure causal attention.

        Mask Visualization for GPU1:

            Global Key Positions:   0    1    2    3    4    5    6    7
            GPU1 Query Positions:         ↑    ↑
                                    (T2) (T3)

            Causal Mask Rules:
            - Q2 (pos 2) can attend to: [0, 1, 2]     → Mask out [3,4,5,6,7] ✓
            - Q3 (pos 3) can attend to: [0, 1, 2, 3]   → Mask out [4,5,6,7]   ✓
    ...
    Step 5: Local Attention Computation
        Each GPU computes:

            GPU1 Local Queries: Q2, Q3 (from tokens T2, T3)
                GPU1 Global Keys:   [K0 K1 K2 K3 K4 K5 K6 K7] (ALL tokens)

                Attention Computation:
                Q2 × [K0 K1 K2 K3 K4 K5 K6 K7] → Proper attention scores
                Q3 × [K0 K1 K2 K3 K4 K5 K6 K7] → Proper attention scores

            With Causal Masking:
                For Q2 (token T2):
                Can attend to: K0, K1, K2           (positions 0,1,2)
                Masked out:    K3, K4, K5, K6, K7   (positions 3-7) ✓

                For Q3 (token T3):
                Can attend to: K0, K1, K2, K3       (positions 0-3)  
                Masked out:    K4, K5, K6, K7       (positions 4-7) ✓

        No Reduce-Scatter Needed:
            Each GPU already has the correct local output - no communication required!

    Step 6: Complete Data Flow

        SEQUENCE DISTRIBUTION:
            Global: [T0 T1 T2 T3 T4 T5 T6 T7]
                GPU0: [T0 T1] → Q0,Q1, K0,K1, V0,V1
                GPU1: [T2 T3] → Q2,Q3, K2,K3, V2,V3  
                GPU2: [T4 T5] → Q4,Q5, K4,K5, V4,V5
                GPU3: [T6 T7] → Q6,Q7, K6,K7, V6,V7

            ALL-GATHER PHASE:
                Each GPU exchanges ALL local K,V → All get COMPLETE global K,V

            ATTENTION PHASE:
                GPU0: [Q0,Q1] × Global_K × Global_V → [Output0,Output1]
                GPU1: [Q2,Q3] × Global_K × Global_V → [Output2,Output3]  
                GPU2: [Q4,Q5] × Global_K × Global_V → [Output4,Output5]
                GPU3: [Q6,Q7] × Global_K × Global_V → [Output6,Output7]

            FINAL OUTPUTS:
                GPU0: [Output0,Output1] (local to T0-T1)
                GPU1: [Output2,Output3] (local to T2-T3) 
                GPU2: [Output4,Output5] (local to T4-T5)
                GPU3: [Output6,Output7] (local to T6-T7)
'''
...
class LLMconfig:
    ...
    '''
    Example:
        Global sequence length: 1024 tokens
        Context parallel size: 4 GPUs
        Each GPU processes: 256 tokens (1024 ÷ 4)
    '''

    '''
        Number of GPUs participating in context parallelism.
        Values:
        1: No context parallelism (default)
        2+: Number of GPUs splitting the sequence dimension
    '''
    context_parallel_size: int = 1  

    '''
        Meaning: Unique identifier for each GPU in the context parallel group.
        
        Values:
        0 to context_parallel_size - 1
        Each GPU gets a unique rank to identify which sequence chunk it processes
        ...
        context_parallel_rank = 3
    '''        
    context_parallel_rank: int = 0

    '''
        Meaning: PyTorch distributed communication group for context parallel GPUs.

        Values:
        None: No group defined (default)
        
        torch.distributed.Group: Communication group handle
        Purpose: Manages which GPUs communicate with each other for context parallel operations.
        # Creates a group containing all GPUs
...
'''
Step 1: Initial Local Computation
    GPU0: Computes K0, V0 from tokens [T0, T1]
    GPU1: Computes K1, V1 from tokens [T2, T3]  
    GPU2: Computes K2, V2 from tokens [T4, T5]
    GPU3: Computes K3, V3 from tokens [T6, T7]

Step 2: All-Gather Exchange
    Each GPU shares its K,V with all other GPUs:

        GPU0 SENDS: K0, V0 → GPU1, GPU2, GPU3
        GPU1 SENDS: K1, V1 → GPU0, GPU2, GPU3  
        GPU2 SENDS: K2, V2 → GPU0, GPU1, GPU3
        GPU3 SENDS: K3, V3 → GPU0, GPU1, GPU2

Step 3: After All-Gather
    GPU0 now has: [K0, K1, K2, K3], [V0, V1, V2, V3]
    GPU1 now has: [K0, K1, K2, K3], [V0, V1, V2, V3]  ← KEY POINT!
    GPU2 now has: [K0, K1, K2, K3], [V0, V1, V2, V3]
    GPU3 now has: [K0, K1, K2, K3], [V0, V1, V2, V3]


How GPU1 Attends to GPU0's Tokens?
Now GPU1 has ALL the information it needs:

For Q2 (token T2) on GPU1:
    GPU1's Local Queries: [Q2, Q3] (from tokens T2, T3)
    GPU1's Global Keys:   [K0, K1, K2, K3] (from ALL tokens T0-T7)

    Attention Computation:
    Q2 × [K0, K1, K2, K3, K4, K5, K6, K7] = Attention scores


GPU1 now has all the information needed to compute:
    Q2 × K0  (attention to token 0)
    Q2 × K1  (attention to token 1) 
    Q2 × K2  (attention to token 2)
'''
class GQA(nn.Module):
...
'''
Context Parallel in NaiveMHLA - Complete Explanation

MHLA vs Standard Attention
    ...
    MHLA Attention:
        Q × k_eff × c_kvᵀ → Attention → × c_kv × v_eff

Where:
    c_kv is the compressed KV representation
    k_eff, v_eff are pre-absorbed weight matrices


Initial Setup (4 GPUs Example)
    Global Sequence: [T0 T1 T2 T3 T4 T5 T6 T7]
    GPU0 (rank=0): [T0 T1]  ← Local chunk
    GPU1 (rank=1): [T2 T3]  ← Local chunk  
    GPU2 (rank=2): [T4 T5]  ← Local chunk
    GPU3 (rank=3): [T6 T7]  ← Local chunk

    Each GPU processes: T_local = 2 tokens
'''
...
class NaiveMHLA(nn.Module):
    ...
    def forward(self, x:torch.Tensor, freqs_cis:torch.Tensor|None, kv_cache=None, VAL_RUN=False):
        '''
        step1 : Local Input Processing

        Each GPU starts with its local tokens:
            GPU0: x = [T0, T1]  shape: (B, 2, C)
            GPU1: x = [T2, T3]  shape: (B, 2, C)
            GPU2: x = [T4, T5]  shape: (B, 2, C)  
            GPU3: x = [T6, T7]  shape: (B, 2, C)
        '''
        B, T_local, C = x.size()  # CHANGE: T_local


        '''
        Step 2: Compute Local Compressed KV
        Each GPU computes its local compressed KV representation:
            GPU0: new_c_kv = c_kv₀  (from T0-T1)
            GPU1: new_c_kv = c_kv₁  (from T2-T3)
            GPU2: new_c_kv = c_kv₂  (from T4-T5)
            GPU3: new_c_kv = c_kv₃  (from T6-T7)
        '''
        ...
        '''
        KEY CONTEXT PARALLEL STEP - All-Gather c_kv
            
            BEFORE all-gather:
                GPU0: c_kv₀ (shape: B, 2, n_kvl)  ← Only tokens T0-T1
                GPU1: c_kv₁ (shape: B, 2, n_kvl)  ← Only tokens T2-T3
                GPU2: c_kv₂ (shape: B, 2, n_kvl)  ← Only tokens T4-T5
                GPU3: c_kv₃ (shape: B, 2, n_kvl)  ← Only tokens T6-T7

            AFTER all-gather:
                GPU0: [c_kv₀ c_kv₁ c_kv₂ c_kv₃] (shape: B, 8, n_kvl) ← ALL tokens!
                GPU1: [c_kv₀ c_kv₁ c_kv₂ c_kv₃] (shape: B, 8, n_kvl) ← ALL tokens!
                GPU2: [c_kv₀ c_kv₁ c_kv₂ c_kv₃] (shape: B, 8, n_kvl) ← ALL tokens!
                GPU3: [c_kv₀ c_kv₁ c_kv₂ c_kv₃] (shape: B, 8, n_kvl) ← ALL tokens!
        '''
        ...
        '''
        Step 4: Compute Local Queries
            GPU0: q = Q₀ (from T0-T1) shape: (B, nh, 2, hs)
            GPU1: q = Q₁ (from T2-T3) shape: (B, nh, 2, hs)  
            GPU2: q = Q₂ (from T4-T5) shape: (B, nh, 2, hs)
            GPU3: q = Q₃ (from T6-T7) shape: (B, nh, 2, hs)
        '''
        ...
        '''
        Step 5: Attention Computation with Global Context

        MHLA Attention Formula:
            attn = Q × k_eff × c_kvᵀ
            
        Visualization for GPU1:
            GPU1 has:
            - Local Q: Q₁ (from T2-T3) 
            - Global c_kv: [c_kv₀ c_kv₁ c_kv₂ c_kv₃] (ALL tokens)

        Computation:
            Q₁ × k_eff × [c_kv₀ c_kv₁ c_kv₂ c_kv₃]ᵀ → Attention scores
        '''
        ...
        '''
        Step 6: Rectangular Causal Masking

        Mask for GPU1 (rank=1, T_local=2):
            Global positions:   0   1   2   3   4   5   6   7
            GPU1 Q positions:         2   3
        ...
        '''
        Step 7: Final Output Computation

            MHLA Output Formula:
                output = Attention × c_kv × v_eff
            
            Each GPU produces LOCAL outputs:
                GPU0: Output₀ (for tokens T0-T1) ← Stays local, no communication needed
                GPU1: Output₁ (for tokens T2-T3) ← Stays local, no communication needed  
                GPU2: Output₂ (for tokens T4-T5) ← Stays local, no communication needed
                GPU3: Output₃ (for tokens T6-T7) ← Stays local, no communication needed
        '''
        ...
        Complete Data Flow Diagram:
            ...
            SEQUENCE DISTRIBUTION:
                ...
                ALL-GATHER PHASE:
                    Each GPU exchanges c_kv → All get [c_kv₀ c_kv₁ c_kv₂ c_kv₃]

                QUERY COMPUTATION (LOCAL):
                    GPU0: [T0 T1] → Q₀
                    GPU1: [T2 T3] → Q₁
                    GPU2: [T4 T5] → Q₂  
                    GPU3: [T6 T7] → Q₃
                ...
                ATTENTION PHASE:
                    GPU0: Q₀ × k_eff × [c_kv₀ c_kv₁ c_kv₂ c_kv₃]ᵀ → Output₀
                    GPU1: Q₁ × k_eff × [c_kv₀ c_kv₁ c_kv₂ c_kv₃]ᵀ → Output₁
                    GPU2: Q₂ × k_eff × [c_kv₀ c_kv₁ c_kv₂ c_kv₃]ᵀ → Output₂
                    GPU3: Q₃ × k_eff × [c_kv₀ c_kv₁ c_kv₂ c_kv₃]ᵀ → Output₃

                FINAL OUTPUTS:
                    GPU0: Output₀ (local to T0-T1)
                    GPU1: Output₁ (local to T2-T3) 
                    GPU2: Output₂ (local to T4-T5)
                    GPU3: Output₃ (local to T6-T7)
        '''
...
Initialization Phase
    ...
    ├─────────────────────────────────────────────────────────────┤
    │ Input Parameters:                                           │
    │   B=2, T=8, file_path="train.bin", device="cuda:0"         │
    │   context_parallel_size=4, context_parallel_rank=1         │
    ├─────────────────────────────────────────────────────────────┤
    │ Calculations:                                               │
    │   local_T = 8 ÷ 4 = 2 tokens per GPU                       │
    │   Memory-map tokens file for efficient reading             │
    │   Validate: 2×8+1=17 tokens fit in dataset                 │
    └─────────────────────────────────────────────────────────────┘

After Initialization:
    DataLoader State:
    ├── Global sequence length (T): 8 tokens
    ├── Local sequence length (local_T): 2 tokens  
    ├── Context parallel size: 4 GPUs
    ├── This GPU's rank: 1 (handles tokens 2-3)
    ├── Memory-mapped file: "train.bin" (entire dataset)
    └── Batch size (B): 2 sequences per batch
'''
class DataLoader:
    ...
    def next_batch(self):
        '''
        Step 1: Generate Random Start Positions
            ┌─────────────────────────────────────────────────────────────┐
            │ start_indices = torch.randint(0, N-8-1, (2,))              │
            │ Example: start_indices = [150, 420] (2 random positions)   │
            └─────────────────────────────────────────────────────────────┘

        Step 2: For Each Start Position - Extract Local Chunk:

            Dataset: [..., token149, token150, token151, ..., token157, ...]
                    (global positions)

            Full sequence extracted: tokens[150:158] = [T150, T151, T152, T153, T154, T155, T156, T157]
                                    (positions 0-7 in local context)
            ...
            GPU1 (rank=1) extracts:
            local_start = 1 × 2 = 2
            local_end   = 2 + 2 = 4

            x_local = tokens[2:4] = [T152, T153]   ← Input tokens
            y_local = tokens[3:5] = [T153, T154]   ← Target tokens (shifted by 1)


        For start=420:

            Full sequence: tokens[420:428] = [T420, T421, T422, T423, T424, T425, T426, T427]

            GPU1 extracts:
            x_local = [T422, T423]   ← Input tokens  
            y_local = [T423, T424]   ← Target tokens

        Step 3: Batch Construction

            After processing both start positions:
        ...
        Complete Multi-GPU Picture:
            All 4 GPUs Working Together:

                Global Sequence: [T150 T151 T152 T153 T154 T155 T156 T157]

                GPU0 (rank=0): 
                x = [[T150,T151], [T420,T421]] 
                y = [[T151,T152], [T421,T422]]

                GPU1 (rank=1): 
                x = [[T152,T153], [T422,T423]]  ← What we just saw
                y = [[T153,T154], [T423,T424]]

                GPU2 (rank=2):
                x = [[T154,T155], [T424,T425]]
                y = [[T155,T156], [T425,T426]]

                GPU3 (rank=3):
                x = [[T156,T157], [T426,T427]] 
                y = [[T157,T158], [T427,T428]]

        Context Parallel Distribution Logic:
            Global sequence split across GPUs:
            ┌─────────┬─────────┬─────────┬─────────┐
            │ GPU0    │ GPU1    │ GPU2    │ GPU3    │
            │ T0-T1   │ T2-T3   │ T4-T5   │ T6-T7   │
            └─────────┴─────────┴─────────┴─────────┘

            Each GPU only processes its assigned chunk:
            • Reduces memory per GPU
            • Enables longer sequences than single GPU can handle


        Async Data Loading:
            ┌─────────────┐    pin_memory()    ┌─────────────┐    non_blocking=True  ┌─────────────┐
            │   CPU RAM   │ ─────────────────→ │ Pinned RAM  │ ────────────────────→ │   GPU RAM   │
            │   x, y      │                    │   x, y      │                       │   x, y      │
            └─────────────┘                    └─────────────┘                       └─────────────┘
                ↓                                  ↓                                    ↓
            Blocking copy                    Non-blocking transfer              GPU computation can
                                            (overlap with compute)              overlap with transfer
...
'''
Function Purpose
    All-Gather along a specific dimension - Efficiently combine tensor chunks from multiple GPUs into full tensors on every GPU.

Overall Flow:
    ┌─────────────────────────────────────────────────────────────┐
    │                   all_gather_sequence()                     │
    │  Input: tensor chunks, Output: full tensor on all GPUs     │
    └─────────────────────────────────────────────────────────────┘
                                │
    ┌──────────────────────────────┼──────────────────────────────┐
    │                              │                              │
    ▼                              ▼                              ▼
    Check Distributed   →   Prepare Tensor      →   All-Gather    →   Restore Shape
    Setup                   for Communication        Operation


Step-by-Step Execution (4 GPUs Example)

Initial State - Each GPU Has Local Chunk:
    GPU0 (rank=0): tensor shape: (2, 4, 8)  [local sequence dim=2]
    GPU1 (rank=1): tensor shape: (2, 4, 8)  [local sequence dim=2]  
    GPU2 (rank=2): tensor shape: (2, 4, 8)  [local sequence dim=2]
    GPU3 (rank=3): tensor shape: (2, 4, 8)  [local sequence dim=2]

    We want to all-gather along DIMENSION 0 (sequence dimension)

Step 1: Check Distributed Setup

Step 2: Dimension Permutation
    Why Permute?
        all_gather_into_tensor always gathers along dimension 0, so we move our target dimension to the front.

    Visualization of Permutation Logic:
        If we were gathering along dim=1 instead:
        Original: (2, 4, 8), dim=1
        perm = [0, 1, 2] → swap 0↔1 → [1, 0, 2]
        t_perm = tensor.permute(1, 0, 2) → shape: (4, 2, 8)
...
Step 3: Prepare Output Buffer
    ...
    Buffer Allocation:
        ┌─────────────────────────────────────────────────┐
        │ out_perm shape: (8, 4, 8)                       │
        │ • 8 = 2 (local) × 4 (GPUs) - total sequence     │
        │ • 4,8 = remaining dimensions                    │
        │ Empty buffer ready to receive gathered data     │
        └─────────────────────────────────────────────────┘
'''
