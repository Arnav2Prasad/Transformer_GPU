...
#         # ✅ SAFETY: Only enable TP if conditions are met
#         self.enable_tp = (enable_tp and 
#                          (self.tp_size > 1) and 
...
#                 gather_output=False, group=self.tp_group  # ✅ Use self.tp_group
#             )
#             self.c_proj = RowParallelLinear(
#                 config.up_dim, config.n_embd, bias=False,
#                 input_is_parallel=True, group=self.tp_group  # ✅ Use self.tp_group
#             )
...
#             # ✅ Ensure non-SwiGLU path uses the activation function
#             x = self.c_fc(x)
#             x = self.non_linearity_func(x)  # ✅ This was missing in some paths
...
# After DDP initialization, around line 1300
if master_process:
    wandb.init(
        project="TP-code-analysis",  # Your project name
        name="TP-code",  # Run name
        config={
            **vars(ModelConfig),
            **vars(TrainingConfig),
            "world_size": world_size,
...
if master_process : 
    ...
    wandb.config.update({
        ...
        "parameter_ratio": active/total if total > 0 else 0
    })
...
for iter in range(TrainingConfig.max_iters+1):
    ...
    for micro_step in range(grad_accum_steps):
        ...
        # # Get next batch (only master loads, then broadcast)
        # if micro_step == grad_accum_steps - 1:  # Preload next batch
        #     if master_process:
        #         x_next, y_next = train_loader.next_batch()
        #     else:
        #         x_next = torch.empty_like(x)
        #         y_next = torch.empty_like(y)
        #     x, y = broadcast_batch(x_next, y_next, src=0)
    ...
    if master_process:
        ...
        wandb.log({
            ...
            "learning_rate": lr,
            "throughput": (B * T * grad_accum_steps) / (dt/1000),  # tokens/sec
            "gpu_memory": mem/1024**3,  # GB
            "time_per_iter": dt
        })
