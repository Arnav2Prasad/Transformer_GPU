...
class LLM(nn.Module):
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device):
        ...
        # ZeRO-1: Replace AdamW with ZeroRedundancyOptimizer
        # Using the simpler version without parameters_as_bucket_view for compatibility
...
'''
DDP Initialization: Each process (GPU) is set up using init_process_group with NCCL as the backend. 
Each process holds an identical copy of the model, which is a key principle of Data Parallelism.
'''
...
class DataLoader:
    ...
    def next_batch(self):
        ...
        '''
        # If B=2, T=3
        x = [[10, 11, 12],    # First sequence
            [20, 21, 22]]    # Second sequence
            
        y = [[11, 12, 13],    # Targets for first sequence  
            [21, 22, 23]]    # Targets for second sequence
        '''
        ...
        '''
        Targeted Transfer: The .to(self.device,…) method ensures this unique data is moved to the correct hardware device.
        self.device holds the specific identifier for the GPU assigned to this process (e.g., cuda:0 for GPU 0, cuda:1 for GPU 1).
        '''
...
'''
The same model architecture is copied to each GPU
Each GPU gets its own replica of the model
device_ids=[ddp_local_rank] ensures each process uses its specific GPU
Gradients are synchronized across GPUs during backward pass

This DistributedDataParallel wrapper effectively makes a copy of the initialized model on every GPU and sets up the necessary communication hooks. 
When the backward pass is executed, the DDP wrapper automatically handles the all-reduce operation to synchronize the gradients across all model copies before the optimizer step.
...
if master_process:
    print(f"ZeRO-1 Optimizer: {type(optimizer).__name__}")
    # Check memory usage
    mem_before = torch.cuda.memory_allocated(device) / 1024**3
    print(f"GPU memory after optimizer creation: {mem_before:.2f} GB")
...
# Each iteration represents one complete global optimization step (equivalent to processing the total_batch_size)
for iter in range(TrainingConfig.max_iters+1):
    ...
    # The gradients accumulated from the previous global step are reset to zero.
    ...
    # grad_accum_steps stores the number of micro-batches
    for micro_step in range(grad_accum_steps):
        '''
        Accumulation: For the first N−1 micro-steps, gradient synchronization is disabled. Each GPU calculates and accumulates gradients only on its own copy of the model, 
        saving significant communication bandwidth.

        Synchronization (Final Step): Only on the final micro-step (micro_step == grad_accum_steps - 1) is synchronization enabled. 
        The DDP wrapper automatically triggers an all-reduce operation during the backward pass. This aggregates the accumulated gradients across all 
        GPUs, resulting in the correct global gradient.
        '''
        ...
        with ctx:
            _, loss, _ = model(x,y)
            '''
            Purpose: This scaling is essential for Gradient Accumulation. 
            Since the backward pass will be called grad_accum_steps times, and the gradients will be summed, dividing the loss by this factor ensures 
            that the final accumulated gradient is mathematically equivalent to the gradient calculated from one single, 
            large batch of size total_batch_size. This keeps the learning rate effective across different accumulation settings.
            '''
        ...
        '''
        Loss Scaling: The loss is multiplied by the scaler (part of AMP) to prevent underflow in float16 when computing small gradients.
        Backward Pass: backward() computes the scaled gradients and accumulates them onto the model parameters.
        '''
