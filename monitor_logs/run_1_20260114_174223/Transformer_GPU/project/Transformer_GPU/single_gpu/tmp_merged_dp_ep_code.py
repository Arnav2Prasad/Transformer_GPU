...
class MoE(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if want_ep==0:
            print('inside forward_single_gpu call()')
            return _forward_single_gpu()
...
ModelConfig = LLMconfig(
    ...
    # EP config (conditionally added)
    **({'ep_size': 1, 'ep_rank': 0, 'ep_group': None} if want_ep == 1 else {}),
